...
import { Shield, Search, FileUp, ArrowLeft, Check, User, DollarSign, BookOpen, LifeBuoy, GanttChartSquare } from 'lucide-react';

// This is the renderer from your App.jsx, which we need to display formatted responses.
// We assume it's been exported from App.jsx or its own file.
// For now, we will copy a simplified version here. If you have it in its own file, import it instead.
            ...
            {parts.map((part, i) => {
                if (sectionRegex.test(part) || standaloneNumberRegex.test(part)) {
                    ...
                    if (sectionNumberMatch) {
                        ...
                        return (
                            <button key={i} onClick={(e) => { e.stopPropagation(); onSectionLinkClick(sectionNumber); }} className="text-blue-300 underline hover:text-blue-200 font-bold focus:outline-none inline">{part}</button>
                        );
                    }
                }
                if (caseLawRegex.test(part)) {
                    const caseName = part.slice(1, -1);
                    return <button key={i} onClick={(e) => { e.stopPropagation(); onLegalLinkClick(caseName); }} className="text-blue-300 underline hover:text-blue-200 font-semibold focus:outline-none inline">{caseName}</button>;
                }
                if (boldRegex.test(part)) {
                    const innerText = part.slice(2, -2);
                    if (statuteKeywords.some(keyword => innerText.includes(keyword))) {
                        return <button key={i} onClick={(e) => { e.stopPropagation(); onLegalLinkClick(innerText); }} className="text-blue-300 underline hover:text-blue-200 font-semibold focus:outline-none inline">{innerText}</button>;
                    }
...
function AIContentRenderer({ content, onSectionLinkClick, onLegalLinkClick }) {
    ...
    if (typeof content === 'string') {
        return (
            <div className="text-white space-y-2 whitespace-pre-line">
    ...
    // Add more complex rendering for objects/arrays if needed, similar to your App.jsx
...
export default function HRSolutionCenter({ apiKey, handbookText, onSectionLinkClick, onLegalLinkClick }) {
    ...
    const handleGenerateSolution = async () => {
        if (!hrQuery && !fileContent) {
            alert("Please describe a scenario or upload a document.");
            return;
        }
        ...
        const prompt = `
        You are 'Navigation IQ,' an expert AI HR consultant for private, independent K-12 schools. Your tone is professional, authoritative, and deeply analytical. Your entire response MUST be a single, valid JSON object.
        ...
        **REQUIRED JSON OUTPUT STRUCTURE:**

        {
          ...
          "actionableRecommendations": "Provide a clear, bulleted list of immediate next steps for the HR Manager. These should be concrete actions."
        }
        `;
    ...
    if (activeCard) {
        // --- Individual Card Analysis View ---
        return (
            <div className="max-w-5xl mx-auto p-4 sm:p-6 text-white">
                <button onClick={() => setActiveCard(null)} className="flex items-center gap-2 text-blue-300 hover:text-blue-200 mb-6 font-semibold">
                    <ArrowLeft size={18} />
                    Back to HR Solutions Center
                </button>

                <div className="bg-[#4B5C64] p-6 sm:p-8 rounded-2xl shadow-2xl">
                    {/* THIS IS THE FIX for the title color */}
                    ...
                    <textarea
                        className="w-full min-h-[120px] p-3 rounded-lg text-black text-base focus:ring-2 focus:ring-blue-400 focus:outline-none transition mb-4"
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gray-600 hover:bg-gray-500 text-white font-semibold px-5 py-2 rounded-lg shadow-md transition-all disabled:bg-gray-700 disabled:cursor-not-allowed"
                        >
                            <FileUp size={18} />
                            Upload Document
                        </button>
                    ...
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-lg shadow-lg text-lg transition-all disabled:bg-gray-500 disabled:cursor-not-allowed"
                    >
                        {isLoading ? loadingMessage : "Generate Solution"}
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {hrCards.map(card => (
                    <button
                        ...
                        className="bg-[#4B5C64] p-6 rounded-2xl shadow-lg text-white text-left hover:bg-gray-700 hover:scale-105 transition-all duration-300"
                    >
