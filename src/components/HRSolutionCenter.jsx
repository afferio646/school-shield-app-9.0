...
// --- MODIFIED: This component now filters archives by the current card's title ---
...
export default function HRSolutionCenter({ apiKey, handbookText, onSectionLinkClick, onLegalLinkClick }) {
    ...
    const hrCards = [
        ...
        { title: "Employee Wage Compliance", description: "Analyze scenarios related to employee classification (exempt vs. non-exempt), overtime calculations, and other Fair Labor Standards Act (FLSA) rules.", icon: <DollarSign size={36} className="text-white" /> },
        { title: "Workplace Manager", description: "Get a step-by-step protocol for conducting fair and thorough investigations into claims of harassment, discrimination, or other misconduct.", icon: <Search size={36} className="text-white" /> },
        { title: "Employee Compliance Checker", description: "Analyze your existing policies to identify potential compliance gaps for remote employees working in different states with varying labor laws.", icon: <Check size={36} className="text-white" /> },
        { title: "Hiring & Background Navigator", description: "Ensure your hiring and background check processes are compliant with the Fair Credit Reporting Act (FCRA) and state-specific 'Ban-the-Box' laws.", icon: <User size={36} className="text-white" /> },
    ...
    if (activeCard) {
        return (
            <div className="max-w-5xl mx-auto p-4 sm:p-6 text-white">
                ...
                {/* MODIFIED: The archive is now rendered here, inside the analysis view, and receives the card title for filtering */}
                <ArchivedResponses archives={archivedResponses} cardTitle={activeCard.title} onView={setViewedArchive} />
    ...
    // This is the main 7-card grid view
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6">
            ...
            {/* The archive is no longer here, it has been moved to the individual card view */}
        </div>
    );
}
